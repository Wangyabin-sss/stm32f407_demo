...
#define GS_SDA_IN()  {GPIOG->MODER&=~(3<<(9*2));GPIOG->MODER|=0<<9*2;}	//PB9输入模式
#define GS_SDA_OUT() {GPIOG->MODER&=~(3<<(9*2));GPIOG->MODER|=1<<9*2;}  //PB9输出模式

//IO操作函数	 
#define GS_IIC_SCL    PAout(6) 		//SCL
#define GS_IIC_SDA    PGout(9) 		//SDA	 
#define GS_READ_SDA   PGin(9) 		    //输入SDA 
